...
const AgentDetailsPage = () => {
    ...
    if (!agent) {
        return <div className="text-center text-red-400">{t('agentNotFound')}</div>;
    }
        ...
        <div className="space-y-6">
            <div>
                ...
                <div className="flex justify-between items-center">
                    ...
                    <button onClick={handleEditAgent} className="text-custom-light-blue hover:text-white" title={t('editAgent')}>
                        <Edit size={20} />
                    </button>
                </div>
                <p className="text-gray-400">{t(agent.role.replace(/\s/g, ''))}</p>
            </div>
            ...
            <div className="bg-gray-800/50 p-4 rounded-lg">
                <div className="flex justify-between items-center mb-4">
                    <h3 className="text-xl font-semibold text-custom-light-blue">{t('assignedCustomers')}</h3>
                    <Link to="/customers" className="flex items-center space-x-2 text-blue-400 hover:underline">
                        <Users size={16} />
                        <span>{t('viewAllCustomers')}</span>
                    </Link>
                </div>
