        ...
        <div>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                <div>
                    ...
                    <select {...register('customerId')} className="form-input bg-gray-800 border-gray-700">
                ...
                <div className="space-y-3">
                    ...
                    {fields.map((field, index) => (
                        <div key={field.id} className="flex items-center space-x-2 bg-gray-800 p-2 rounded-lg">
                            <select {...register(`items.${index}.productId`)} className="form-select bg-gray-700 border-gray-600 flex-grow">
                                {products.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}
                            </select>
                            <input type="number" {...register(`items.${index}.quantity`)} className="form-input bg-gray-700 border-gray-600 w-24" min="1" />
