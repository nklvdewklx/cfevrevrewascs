...
const AgentOrderForm = () => {
    ...
    const onSubmit = (data) => {
        ...
        showToast(t('newOrderCreated'), 'success');
        navigate('/agent/home');
    };
    ...
    return (
        <div>
            ...
            <h2 className="text-3xl font-bold text-white mb-6">{t('createNewOrder')}</h2>
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                <div>
                    <label className="block mb-2 text-sm text-gray-400">{t('customer')}</label>
                    <select {...register('customerId', { required: t('customerRequired') })} className="form-input bg-gray-800 border-gray-700">
                ...
                <div className="space-y-3">
                    <label className="block text-sm text-gray-400">{t('products')}</label>
